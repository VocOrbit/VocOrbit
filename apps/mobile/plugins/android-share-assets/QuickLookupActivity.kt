...
class QuickLookupActivity : Activity() {
  ...
  private fun buildContentView(): View {
    return ScrollView(this).apply {
      ...
      addView(
        LinearLayout(this@QuickLookupActivity).apply {
  ...
  private fun buildHeader(): View {
    return LinearLayout(this).apply {
      ...
      addView(
        View(this@QuickLookupActivity),
        LinearLayout.LayoutParams(0, 1, 1f),
      )

      addView(
        TextView(this@QuickLookupActivity).apply {
          text = "X Close"
  ...
  private fun buildBrandView(): View {
    return LinearLayout(this).apply {
      ...
      addView(
        ImageView(this@QuickLookupActivity).apply {
  ...
  private fun buildLoadingRow(): View {
    loadingRow = LinearLayout(this).apply {
      ...
      addView(ProgressBar(this@QuickLookupActivity).apply { isIndeterminate = true })
      addSpaceHorizontal(this, 10)
      addView(bodyTextView("VocOrbit is checking the basic meaning..."))
    }
    return loadingRow
  }
  ...
  private fun runLookup() {
    ...
    val sentence = QuickLookupTextSupport.buildContextSnippet(payload.text, selected, maxLength = 280)
    executor.execute {
      try {
    ...
    if (error is QuickLookupApiException) {
      return when (error.code) {
        ...
        -> UiError(
          ...
          body = "Open VocOrbit and sign in again. Then quick lookup will work here too.",
        )

        QuickLookupApiException.Code.FORBIDDEN -> UiError(
          title = "Basic credits required",
          body = "You are out of basic credits. Open Billing & credits in VocOrbit to continue.",
        )

        QuickLookupApiException.Code.TIMEOUT -> UiError(
          title = "Lookup timed out",
          body = "This quick lookup took too long. Try again or open VocOrbit if you want to continue there.",
        )
  ...
  companion object {
    private const val EXTRA_TEXT = "vocorbit.share.text"
    private const val EXTRA_SOURCE = "vocorbit.share.source"
    private const val EXTRA_RECEIVED_AT = "vocorbit.share.received_at"
    private const val EXTRA_SELECTED_WORD = "vocorbit.share.selected_word"
    private const val EXTRA_NEEDS_OVERLAY_PERMISSION = "vocorbit.share.needs_overlay_permission"

    fun createIntent(context: Context, payload: SharePayload): Intent {
      return Intent(context, QuickLookupActivity::class.java).apply {
