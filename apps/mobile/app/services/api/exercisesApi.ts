...
export type ExerciseQuestionType = "meaning_match" | "fill_in_gap" | "guess_word" | "match_synonym"
export type ExerciseSessionStatus = "active" | "completed"
export type ExerciseCatalogAvailabilityReason = "not_enough_learning_items" | "missing_synonyms"
...
class ExercisesApi {
  ...
  async getCatalog(input?: {
    ...
    return backendApiClient.get<ExercisePracticeCatalog>("/v1/exercises/catalog", {
      mode: input?.mode,
      timezoneOffsetMinutes: input?.timezoneOffsetMinutes,
      groupIds: input?.groupIds && input.groupIds.length > 0 ? input.groupIds.join(",") : undefined,
    })
  }
  ...
  ): Promise<ApiResult<ExerciseSessionDetail>> {
