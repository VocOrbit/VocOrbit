...
export function mapLearningItemToEntry(item: WordInsightLearningItem): VocabularyEntry {
  ...
  const pronunciation = resolveDisplayPhonetic(item.phonetic, item.lemma ?? item.vocab)
  const contextSentence = readNonEmptyString(item.contextSentence)
...
export function applyWordInsightToEntryDetail(
  detail: VocabularyEntryDetail,
  input: ApplyWordInsightInput,
): VocabularyEntryDetail {
  if (!isRecord(input.insight)) {
    return {
      ...detail,
      entry: {
        ...detail.entry,
  ...
  const insight = input.insight as InsightPayload & Record<string, unknown>
  const detailsPayload = isRecord(insight.details) ? insight.details : undefined
  const contextSentence =
    readNonEmptyString(input.sentence) ?? detail.contextSentence ?? detail.entry.contextSentence
  const exampleSentence = readNonEmptyString(insight.exampleSentence)
  const sentence = contextSentence ?? exampleSentence ?? detail.sentence
  const translatedExample = readNonEmptyString(insight.translatedExample) ?? detail.translatedExample
  const surface =
    readNonEmptyString(insight.surface) ??
    readNonEmptyString(insight.word) ??
    readNonEmptyString(input.selectedWord) ??
    detail.entry.word
  const lemma = readNonEmptyString(insight.lemma) ?? detail.entry.lemma
  const phonetic = readNonEmptyString(insight.phonetic) ?? detail.entry.pronunciation
  const sourceLang = readNonEmptyString(insight.sourceLang) ?? detail.entry.sourceLang
  const targetLang = readNonEmptyString(insight.targetLang) ?? detail.entry.targetLang
  const partOfSpeech = readNonEmptyString(insight.partOfSpeech) ?? detail.entry.partOfSpeech
  const translationL1 =
    readNonEmptyString(insight.translationL1) ??
    readNonEmptyString(insight.meaning) ??
    detail.nativeMeaning ??
    detail.meaning ??
    detail.entry.definition
  const definitionL2 =
    readNonEmptyString(insight.definitionL2) ??
    readNonEmptyString(insight.sourceMeaning) ??
    detail.learningDefinition
  const whyThisSense =
    readNonEmptyString(insight.whyThisSense) ??
    readNonEmptyString(insight.shortExplanation) ??
  ...
  const synonyms = readStringArray(insight.synonyms)
  const alternativeMeanings = readStringArray(
    detailsPayload?.alternativeMeanings ?? insight.alternativeMeanings,
  )
  const antonyms = readStringArray(detailsPayload?.antonyms ?? insight.antonyms)
  const usageNotes = readStringArray(detailsPayload?.usageNotes ?? insight.usageNotes)
  const collocations = readStringArray(detailsPayload?.collocations ?? insight.collocations)
  const confidence = readFiniteNumber(insight.confidence) ?? detail.confidence
  const examples = contextSentence
    ? uniqueStrings([exampleSentence, translatedExample])
    : uniqueStrings([sentence, translatedExample])
  ...
  return {
    ...
    entry: {
      ...detail.entry,
      word: surface,
      pronunciation: resolveDisplayPhonetic(phonetic, lemma || surface),
      partOfSpeech,
      definition: translationL1,
      example: definitionL2 ?? whyThisSense ?? translationL1,
      examples: examples.length > 0 ? examples : detail.entry.examples,
      contextSentence,
      lemma,
      sourceLang,
      targetLang,
    ...
    contextSentence,
    learningDefinition: definitionL2 ?? sentence ?? detail.learningDefinition,
    nativeMeaning: translationL1,
    sentence,
    translatedExample,
    synonyms: synonyms.length > 0 ? synonyms : detail.synonyms,
    alternativeMeanings:
      alternativeMeanings.length > 0 ? alternativeMeanings : detail.alternativeMeanings,
    antonyms: antonyms.length > 0 ? antonyms : detail.antonyms,
    usageNotes: usageNotes.length > 0 ? usageNotes : detail.usageNotes,
    collocations: collocations.length > 0 ? collocations : detail.collocations,
    confidence,
    meaning: translationL1,
    shortExplanation: whyThisSense,
  }
