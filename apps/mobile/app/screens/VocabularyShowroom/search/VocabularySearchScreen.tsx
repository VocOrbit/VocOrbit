...
export const VocabularySearchScreen: FC<VocabularySearchScreenProps> = ({ navigation, route }) => {
  const { themed, theme } = useAppTheme()
  const { setSession } = useAuth()
  const showroomColors = theme.colors.vocabularyShowroom
  const $bottomInsets = useSafeAreaInsetsStyle(["bottom"])
  const targetGroupId = route.params?.targetGroupId?.trim() || undefined
  const targetGroupName = route.params?.targetGroupName?.trim() || undefined
  ...
  const [groupErrorMessage, setGroupErrorMessage] = useState<string | undefined>(undefined)
  const [pronunciationErrorMessage, setPronunciationErrorMessage] = useState<string | undefined>(undefined)
  const [requiresLogin, setRequiresLogin] = useState(false)
  const [groupMutatingItemId, setGroupMutatingItemId] = useState<string | undefined>(undefined)
  ...
  const handleAddToGroup = useCallback(
    (itemId: string) => {
      if (!targetGroupId || groupMutatingItemId || addedItemIds.includes(itemId)) return

      setGroupMutatingItemId(itemId)
      setGroupErrorMessage(undefined)
      void (async () => {
        try {
          const response = await wordInsightApi.addLearningItemToGroup(targetGroupId, itemId)
          if (response.kind === "ok") {
            setAddedItemIds((prev) => (prev.includes(itemId) ? prev : [...prev, itemId]))
            return
          }

          setGroupErrorMessage(
            response.kind === "not-found"
              ? translate("vocabulary:errors.groupNotFound")
          ...
          setGroupMutatingItemId(undefined)
        }
      })()
    },
    [addedItemIds, groupMutatingItemId, targetGroupId],
  )
  ...
  return (
    ...
    >
      ...
      <View style={themed($layout)}>
        ...
        {targetGroupId ? (
          <Text
            style={themed($groupHintText)}
            text={translate("vocabulary:showroom.groupAddTo", {
              name: targetGroupName ?? translate("vocabulary:showroom.groupsTitle"),
            })}
          />
        ) : null}
        {groupErrorMessage ? (
          <Text style={themed($groupErrorText)} text={groupErrorMessage} />
        ) : null}
                ...
                <View style={themed($card)}>
                  ...
                  {targetGroupId ? (
                    <Pressable
                      ...
                      disabled={groupMutatingItemId === item.id || addedItemIds.includes(item.id)}
                      accessibilityRole="button"
                      accessibilityLabel={translate("vocabulary:showroom.addToGroupButton")}
                      style={({ pressed }) => [
                        themed($addGroupButton),
                        (groupMutatingItemId === item.id || addedItemIds.includes(item.id)) &&
                          themed($addGroupButtonDisabled),
                        pressed &&
                          groupMutatingItemId !== item.id &&
                          !addedItemIds.includes(item.id) &&
                          themed($addGroupButtonPressed),
                      ]}
                    >
                      <MaterialCommunityIcons name="plus" size={16} color="#FFFFFF" />
                      <Text
                        style={themed($addGroupButtonText)}
                        text={
                          addedItemIds.includes(item.id)
                            ? translate("vocabulary:showroom.addedToGroupButton")
                            : translate("vocabulary:showroom.addToGroupButton")
                        }
                      />
                    </Pressable>
                  ) : null}
...
const $groupHintText: ThemedStyle<TextStyle> = ({ spacing, typography, colors }) => ({
  marginTop: spacing.xs,
  fontFamily: typography.primary.medium,
  fontSize: 12,
...
const $groupErrorText: ThemedStyle<TextStyle> = ({ spacing, colors, typography }) => ({
  marginTop: spacing.xs,
  fontFamily: typography.primary.medium,
  fontSize: 12,
  color: colors.error,
...
const $addGroupButton: ThemedStyle<ViewStyle> = ({ spacing, colors }) => ({
  marginTop: spacing.sm,
  minHeight: 42,
  borderRadius: 12,
...
const $addGroupButtonPressed: ThemedStyle<ViewStyle> = ({ colors }) => ({
  opacity: 0.86,
  backgroundColor: colors.vocabularyShowroom.accent,
})

const $addGroupButtonDisabled: ThemedStyle<ViewStyle> = () => ({
  opacity: 0.5,
})

const $addGroupButtonText: ThemedStyle<TextStyle> = ({ typography }) => ({
  fontFamily: typography.primary.semiBold,
  fontSize: 14,
  color: "#FFFFFF",
})
