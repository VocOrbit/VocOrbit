...
import type {
  FillInGapAnswer,
  FillInGapAnswerState,
  FillInGapQuestion,
  VocabularyFillInGapPracticeViewModel,
} from "./useVocabularyFillInGapPracticeViewModel"
...
type VocabularyFillInGapPracticeViewProps = VocabularyFillInGapPracticeViewModel & {
  onRequestLeave: () => void
  onRequestLogin: () => void
...
const resolveOptionTone = (
  answer: FillInGapAnswer,
  question: FillInGapQuestion,
  selectedId: string | null,
  answerState: FillInGapAnswerState,
): OptionTone => {
...
export const VocabularyFillInGapPracticeView: FC<VocabularyFillInGapPracticeViewProps> = ({
  ...
  const [resultOverlayHeight, setResultOverlayHeight] = useState(0)

  const isResultVisible = answerState !== "idle"
  const resultLift = Math.max(220, resultOverlayHeight)
  const scrollBottomPadding =
    theme.spacing.xxl + bottomInset + (isResultVisible ? resultLift + theme.spacing.md : 0)
  ...
  const gapText =
    answerState === "idle" ? question.placeholder : correctAnswer?.label ?? question.placeholder
  ...
  return (
    ...
    >
      ...
      <View style={themed($headerRow)}>
        ...
        <View style={themed($headerSpacer)}>
          <AppTutorialVideoButton
            screen="vocabulary_fill_in_gap_practice"
            placement="overview"
            variant="help"
            ...
            <View style={themed($questionStack)}>
              ...
              <View style={themed($questionCard)}>
                <Text style={themed($questionText)}>
                  ...
                  <Text
                    style={
                      answerState === "idle" ? themed($questionPlaceholder) : themed($questionBlank)
                    }
                    text={gapText}
                  />
                  {question.suffix}
      ...
      <Animated.View
        onLayout={(event) => setResultOverlayHeight(event.nativeEvent.layout.height)}
      ...
      >
        <View style={themed($resultCard)}>
          ...
          <Text style={themed($resultDefinition)} text={`Definition: ${question.definition}`} />
...
const $questionBlank: ThemedStyle<TextStyle> = ({ typography, colors }) => ({
  fontFamily: typography.primary.semiBold,
  textDecorationLine: "underline",
  textDecorationColor: colors.vocabularyShowroom.textStrong,
})

const $questionPlaceholder: ThemedStyle<TextStyle> = ({ typography, colors }) => ({
  fontFamily: typography.primary.semiBold,
  color: colors.vocabularyShowroom.textStrong,
})
...
const $resultDefinition: ThemedStyle<TextStyle> = ({ typography, colors }) => ({
  marginTop: 8,
  fontFamily: typography.primary.normal,
  fontSize: 14,
  lineHeight: 20,
