...
type VocabularyPracticeViewProps = VocabularyPracticeViewModel & {
  onRequestBack: () => void
  onRequestPracticeDetail: (practiceId: PracticeTileId) => void
  onRequestLogin: () => void
  onRequestOpenIap: () => void
  onRequestOpenShowroom: () => void
  onRequestOpenGroups: () => void
}
...
const PracticeTile: FC<PracticeTileProps> = ({
  ...
  const hintText = available
    ...
    : reason === "missing_synonyms"
      ...
      : minimumItemCount <= 1
        ? translate("vocabulary:practiceHub.hints.minOneActiveWord")
    ...
    >
      ...
      >
        ...
        <View style={themed($section)}>
          ...
          <View style={themed($sourceBlock)}>
            <Text
              style={themed($sourceLabel)}
              text={translate("vocabulary:practiceHub.sourceLabel")}
            />
            <Pressable
              onPress={onRequestOpenGroups}
              accessibilityRole="button"
              accessibilityLabel={translate("vocabulary:showroom.accessibility.openGroups")}
              style={({ pressed }) => [
                themed($sourceSummaryCard),
                pressed && themed($sourceSummaryCardPressed),
              ]}
            >
              <Text style={themed($sourceSummaryTitle)} text={sourceLabel} numberOfLines={1} />
              <Text
                style={themed($sourceMutedText)}
                text={translate("vocabulary:practiceHub.selectedSourceCount", {
...
const $sourceBlock: ThemedStyle<ViewStyle> = ({ spacing }) => ({
  marginBottom: spacing.md,
})

const $sourceLabel: ThemedStyle<TextStyle> = ({ typography, colors }) => ({
  fontFamily: typography.primary.medium,
  fontSize: 12,
  letterSpacing: 1.1,
  textTransform: "uppercase",
...
const $sourceSummaryCard: ThemedStyle<ViewStyle> = ({ spacing, colors }) => ({
  marginTop: spacing.xs,
  borderRadius: 16,
  borderWidth: 1,
  borderColor: colors.vocabularyShowroom.outline,
...
const $sourceSummaryCardPressed: ThemedStyle<ViewStyle> = ({ colors }) => ({
  backgroundColor: colors.vocabularyShowroom.surfaceStrong,
})

const $sourceSummaryTitle: ThemedStyle<TextStyle> = ({ typography, colors }) => ({
  fontFamily: typography.primary.semiBold,
  fontSize: 16,
  color: colors.vocabularyShowroom.textStrong,
})

const $sourceMutedText: ThemedStyle<TextStyle> = ({ spacing, typography, colors }) => ({
