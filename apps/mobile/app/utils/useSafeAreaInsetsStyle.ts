...
function resolveInsetValue(insets: Record<Edge, number>, edge: Edge): number {
  if (edge === "top" && Platform.OS === "android" && insets.top === 0) {
...
>(
  ...
  return safeAreaEdges.reduce((acc, e) => {
    ...
    return { ...acc, [`${property}${propertySuffixMap[e]}`]: resolveInsetValue(insets, value) }
