...
import { scaleTypographyStyleForIpad } from "./typographyScale"
...
export const ThemeProvider: FC<PropsWithChildren<ThemeProviderProps>> = ({
  ...
  const themeContextOverride: ThemeContextModeT = useMemo(() => {
    if (initialContext) return initialContext
    return themeScheme === "light" || themeScheme === "dark" ? themeScheme : undefined
  ...
  const themeContext: ImmutableThemeContextModeT = useMemo(() => {
    const t = themeContextOverride || (!!systemColorScheme ? systemColorScheme : "light")
    return t === "dark" ? "dark" : "light"
  }, [themeContextOverride, systemColorScheme])
  ...
  const themed = useCallback(
    <T,>(styleOrStyleFn: AllowedStylesT<T>) => {
      ...
      const resolvedStyle = Object.assign({}, ...stylesArray) as T
      return scaleTypographyStyleForIpad(resolvedStyle)
    },
    [theme],
  )
