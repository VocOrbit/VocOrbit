...
function applyUiLocaleFromL1(l1Language?: string) {
  if (!i18n.isInitialized) return

  const nextLocale = resolveSupportedUiLocale(l1Language)
...
export const LanguagePreferencesProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    setCrashLanguagePair(preferences?.l1Language, preferences?.l2Language)
  }, [preferences?.l1Language, preferences?.l2Language])
  ...
  const refreshPreferences = useCallback(async () => {
    ...
    if (response.kind === "ok") {
      ...
      applyUiLocaleFromL1(nextPreferences.l1Language)
    ...
    setPreferences((prev) => {
      ...
      applyUiLocaleFromL1(fallback.l1Language)
      return fallback
    })
    setIsLoading(false)
  ...
  useEffect(() => {
    ...
    const initialOnboardingCompleted = readOnboardingCompleted(userId)
    setIsResolved(false)
    setHasCompletedOnboarding(initialOnboardingCompleted)
    setIsLoading(true)
    ...
    void (async () => {
      ...
      if (response.kind === "ok") {
        ...
        setHasCompletedOnboarding(initialOnboardingCompleted || response.kind !== "not-found")
      }
