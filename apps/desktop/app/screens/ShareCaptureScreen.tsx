  ...
  switch (problem.kind) {
    ...
    case "forbidden":
      return {
        ...
        body: problem.message || "Your account cannot run basic insight right now. Open app settings to continue.",
        allowOpenIap: true,
      }
    case "timeout":
      return {
  ...
  async function handleRunInsight() {
    ...
    try {
      ...
      if (response.data.status !== "completed") {
        ...
        setAllowOpenIap(response.data.errorCode?.toLowerCase().includes("credit") ?? false)
        return
      }
