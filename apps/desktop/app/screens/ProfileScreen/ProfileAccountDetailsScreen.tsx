...
function buildAccountDeletionCopy() {
  return {
    sectionTitle: translate("vocabulary:accountDetails.deleteAccount.sectionTitle"),
    sectionBody: translate("vocabulary:accountDetails.deleteAccount.sectionBody"),
    action: translate("vocabulary:accountDetails.deleteAccount.action"),
    deleting: translate("vocabulary:accountDetails.deleteAccount.deleting"),
    confirmTitle: translate("vocabulary:accountDetails.deleteAccount.confirmTitle"),
    confirmBody: translate("vocabulary:accountDetails.deleteAccount.confirmBody"),
    successTitle: translate("vocabulary:accountDetails.deleteAccount.successTitle"),
    successBody: translate("vocabulary:accountDetails.deleteAccount.successBody"),
    missingUser: translate("vocabulary:accountDetails.deleteAccount.missingUser"),
    cannotConnect: translate("vocabulary:accountDetails.deleteAccount.cannotConnect"),
    sessionExpired: translate("vocabulary:accountDetails.deleteAccount.sessionExpired"),
    failed: translate("vocabulary:accountDetails.deleteAccount.failed"),
    accessibilityLabel: translate("vocabulary:accountDetails.accessibility.deleteAccount"),
  } as const
}

function resolveDeleteAccountErrorMessage(
  problem: GeneralApiProblem,
  copy: ReturnType<typeof buildAccountDeletionCopy>,
): string {
  if (problem.kind === "unauthorized") {
    return copy.sessionExpired
  }
  if (problem.kind === "cannot-connect" || problem.kind === "timeout") {
    return copy.cannotConnect
  }
...
export const ProfileAccountDetailsScreen: FC<ProfileAccountDetailsScreenProps> = ({ navigation }) => {
  ...
  const accountDeletionCopy = buildAccountDeletionCopy()
  ...
  const deleteAccount = useCallback(async () => {
    ...
    setDeleteErrorMessage(resolveDeleteAccountErrorMessage(response, accountDeletionCopy))
  }, [accountDeletionCopy, setSession, userId])
  ...
  const confirmDeleteAccount = useCallback(() => {
    Alert.alert(accountDeletionCopy.confirmTitle, accountDeletionCopy.confirmBody, [
      ...
      {
        text: accountDeletionCopy.action,
        style: "destructive",
        onPress: () => {
          void deleteAccount()
        },
